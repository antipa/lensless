function [xhat, f] = admm2d_solver(y,h,sett_file,varargin)
...
% tau : override regularizer parameter from settings file. If input is
% passed in here, it'll clobber the value set by the file.
h1 = figure(1);
clf
tic
...
% If extra arg passed in, overwrite tau
if ~isempty(varargin)
    tau = varargin{1};
end

%Cast tau to match input data type
tau = cast(tau,'like',y);
...
while n<niter
    ...
    if mod(n,disp_interval)==0
        ...
        imagesc(crop(sk));
        hold on
        caxis([0 prctile(gather(sk(:)),99.9)])
        axis image
        colormap gray
        hold off
        drawnow        
